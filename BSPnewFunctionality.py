  ...
  for i in range(len(Listed_Passwords)):
    if Username == Listed_Users[i] and Encoded_Password == Listed_Passwords[i]:
      ...
      try:
        ...
        Current_User.write("0,Hello User! The number at the start means your access level. It can range from 0 to 10; 0 being the lowest and 10 being the highest!")
        Current_User.close()
      Current_User = open(f"User_{i+1}.txt", "r")
      ...
      if Username == "USSB":
          ...
          if Current_User_Data[2] == "Affirmed User Deletion":
            ...
            if Selective_Clear == "y":
              ...
              for i in range(len(Listed_Passwords)):
                if Username != Listed_Users[i]:
                  os.remove(f"User_{i+1}.txt")
          elif Current_User_Data[2] == "Change User Permissions":
            print("Change User Permissions Affirmed")
            User_Permission_Change = input("Which Users permission is to be changed? ")
            if User_Permission_Change == "USSB":
              print("USSB has a set Permission level which cant be changed.")
            elif User_Permission_Change != "USSB":
              New_Perm_Level = input("What is the new permission level? ")
              for i in range(len(Listed_Users)):
                if User_Permission_Change == Listed_Users[i]:
                  ...
                  Current_User.close()
                  try:
                    Current_User_Data[0] = f"{New_Perm_Level}, "
                  except SyntaxError:
        ...
        print(", ".join(Current_User_Data))
    elif not Logged_in and not message_shown:
      message_shown = True
